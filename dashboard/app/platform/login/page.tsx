...
import { Activity, Lock, Mail, AlertCircle, ArrowLeft, ShieldCheck } from "lucide-react";
    ...
    <main className="min-h-screen flex flex-col justify-center bg-slate-950 px-4 py-12 relative overflow-hidden">
      ...
      <div className="z-10 sm:mx-auto sm:w-full sm:max-w-md bg-slate-900/60 backdrop-blur-md border border-slate-800 p-8 rounded-2xl shadow-xl">
        ...
        <div className="text-center space-y-2 mb-8">
          ...
          <h2 className="text-xl font-bold text-slate-200 flex items-center justify-center space-x-1.5">
            <ShieldCheck className="h-5 w-5 text-emerald-400" />
            <span>SaaS Admin Portal</span>
          </h2>
          <p className="text-sm text-slate-400">
            Log in to manage tenant accounts, provision owners, and check system health.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="admin@monitra.local"
                className="w-full bg-slate-950 border border-slate-800 rounded-lg py-3 pl-10 pr-4 text-sm text-white placeholder-slate-600 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition duration-200"
              />
            </div>
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-medium py-3 rounded-lg text-sm shadow-md hover:shadow-emerald-500/20 active:bg-emerald-700 transition duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
          >
            {loading ? (
              <span className="inline-block h-4 w-4 border-2 border-white/30 border-t-white rounded-full animate-spin mr-2" />
            ) : null}
            <span>{loading ? "Authorizing..." : "Access Operator Console"}</span>
          </button>
        </form>

        <div className="mt-8 text-center text-xs text-slate-500">
          <p>Local Development Seed: admin@monitra.local / AdminPass123!</p>
        </div>
      </div>
    </main>
  );
